...
void main() {
  group('CBOR Codec - RFC 7049 Compliance Tests', () {
    ...
    group('Unsigned integers', () {
      test('0 to 23', () {
        expect(bytesToHex(CborCodec.encode(0)), equals('00'));
        expect(bytesToHex(CborCodec.encode(1)), equals('01'));
        expect(bytesToHex(CborCodec.encode(10)), equals('0a'));
        expect(bytesToHex(CborCodec.encode(23)), equals('17'));
      });

      test('24 to 255', () {
        expect(bytesToHex(CborCodec.encode(24)), equals('1818'));
        expect(bytesToHex(CborCodec.encode(25)), equals('1819'));
        expect(bytesToHex(CborCodec.encode(100)), equals('1864'));
        expect(bytesToHex(CborCodec.encode(255)), equals('18ff'));
      });

      test('256 to 65535', () {
        expect(bytesToHex(CborCodec.encode(256)), equals('190100'));
        expect(bytesToHex(CborCodec.encode(1000)), equals('1903e8'));
        expect(bytesToHex(CborCodec.encode(65535)), equals('19ffff'));
      });

      test('65536 to 4294967295', () {
        expect(bytesToHex(CborCodec.encode(65536)), equals('1a00010000'));
        expect(bytesToHex(CborCodec.encode(1000000)), equals('1a000f4240'));
        expect(bytesToHex(CborCodec.encode(4294967295)), equals('1affffffff'));
      });

      test('4294967296 and above', () {
        expect(bytesToHex(CborCodec.encode(4294967296)),
            equals('1b0000000100000000'));
        expect(bytesToHex(CborCodec.encode(1000000000000)),
    ...
    group('Negative integers', () {
      test('-1 to -24', () {
        expect(bytesToHex(CborCodec.encode(-1)), equals('20'));
        expect(bytesToHex(CborCodec.encode(-10)), equals('29'));
        expect(bytesToHex(CborCodec.encode(-24)), equals('37'));
      });

      test('-25 to -256', () {
        expect(bytesToHex(CborCodec.encode(-25)), equals('3818'));
        expect(bytesToHex(CborCodec.encode(-100)), equals('3863'));
        expect(bytesToHex(CborCodec.encode(-256)), equals('38ff'));
      });

      test('-257 to -65536', () {
        expect(bytesToHex(CborCodec.encode(-257)), equals('390100'));
        expect(bytesToHex(CborCodec.encode(-1000)), equals('3903e7'));
        expect(bytesToHex(CborCodec.encode(-65536)), equals('39ffff'));
      });
    });

    group('Floating-point numbers', () {
      test('Floating-point numbers', () {
        final bytes = CborCodec.encode(3.14159);
        expect(CborCodec.decode(bytes), closeTo(3.14159, 0.00001));

        final bytes2 = CborCodec.encode(1.0e+300);
        expect(CborCodec.decode(bytes2), equals(1.0e+300));

        final bytes3 = CborCodec.encode(1.0e-300);
        expect(CborCodec.decode(bytes3), equals(1.0e-300));
      });
    });

    group('Simple values', () {
      test('Boolean values', () {
        expect(bytesToHex(CborCodec.encode(false)), equals('f4'));
        expect(bytesToHex(CborCodec.encode(true)), equals('f5'));
      });

      test('Null value', () {
        expect(bytesToHex(CborCodec.encode(null)), equals('f6'));
      });
    });

    group('Strings', () {
      test('Empty string', () {
        expect(bytesToHex(CborCodec.encode('')), equals('60'));
      });

      test('ASCII strings', () {
        expect(bytesToHex(CborCodec.encode('a')), equals('6161'));
        expect(bytesToHex(CborCodec.encode('IETF')), equals('6449455446'));
        expect(bytesToHex(CborCodec.encode('hello')), equals('6568656c6c6f'));
      });

      test('Unicode strings', () {
        expect(bytesToHex(CborCodec.encode('привет')),
            equals('6cd0bfd180d0b8d0b2d0b5d182'));
        expect(bytesToHex(CborCodec.encode('☺')), equals('63e298ba'));
      });

      test('Longer strings', () {
        final longStr = 'a' * 24;
        final encoded = CborCodec.encode(longStr);
        expect(encoded[0],
            equals(0x78)); // начинается с 0x78 (строка, длина в 1 байт)
        expect(encoded[1], equals(24)); // длина 24
        expect(CborCodec.decode(encoded), equals(longStr));

        final veryLongStr = 'a' * 1000;
        final encoded2 = CborCodec.encode(veryLongStr);
        expect(encoded2[0],
            equals(0x79)); // начинается с 0x79 (строка, длина в 2 байта)
        expect(CborCodec.decode(encoded2), equals(veryLongStr));
      });
    });

    group('Arrays', () {
      test('Empty array', () {
        expect(bytesToHex(CborCodec.encode([])), equals('80'));
      });

      test('Small arrays', () {
        expect(bytesToHex(CborCodec.encode([1, 2, 3])), equals('83010203'));
        expect(
            bytesToHex(CborCodec.encode([
              1,
              [2, 3],
              [4, 5]
            ])),
      ...
      test('Longer arrays', () {
        ...
        final encoded = CborCodec.encode(longArray);
        expect(encoded[0],
            equals(0x98)); // начинается с 0x98 (массив, длина в 1 байт)
        expect(encoded[1], equals(100)); // длина 100
        expect(CborCodec.decode(encoded), equals(longArray));
      });
    });

    group('Maps', () {
      test('Empty map', () {
        expect(bytesToHex(CborCodec.encode({})), equals('a0'));
      });
      ...
      test('Maps with non-string keys', () {
        final encoded = CborCodec.encode({1: 'a', 2: 'b', 3: 'c'});
        final decoded = CborCodec.decode(encoded);
    ...
    group('Byte strings', () {
      test('Byte strings', () {
        ...
        final encoded = CborCodec.encode(bytes);
    ...
    group('RFC 7049 Appendix A examples', () {
      test('Examples from RFC 7049 Appendix A', () {
        ...
        for (var example in examples) {
          ...
          final decoded = CborCodec.decode(hexToBytes(hexExpected));
    ...
    group('Indefinite length', () {
      test('Array with indefinite length', () {
        final bytes = CborCodec.encodeIndefiniteArray([1, 2, 3]);
        final decoded = CborCodec.decode(bytes);
        expect(decoded, equals([1, 2, 3]));
      });

      test('Map with indefinite length', () {
        final bytes = CborCodec.encodeIndefiniteMap({'a': 1, 'b': 2});
        final decoded = CborCodec.decode(bytes);
        expect(decoded, equals({'a': 1, 'b': 2}));
      });
    });
  });
}
