...
void main() {
  group('Тестирование диагностических методов', () {
    late RpcEndpoint clientEndpoint;
    late RpcEndpoint serverEndpoint;
    late DiagnosticServiceClient diagnosticClient;
    late DiagnosticServiceServer diagnosticServer;

    setUp(() {
      // Создаем пару эндпоинтов напрямую
      final endpoints = TestFixtureUtils.createEndpointPair();
      clientEndpoint = endpoints.client;
      serverEndpoint = endpoints.server;

      // Создаем и регистрируем диагностические контракты
      diagnosticServer = DiagnosticServiceServer();
      diagnosticClient = DiagnosticServiceClient(clientEndpoint);

      // Регистрируем контракты на соответствующих эндпоинтах
      serverEndpoint.registerServiceContract(diagnosticServer);
      clientEndpoint.registerServiceContract(diagnosticClient);
    });
