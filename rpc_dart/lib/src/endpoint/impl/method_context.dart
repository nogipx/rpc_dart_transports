...
abstract class MetadataUpdatable {
  /// Создает новый экземпляр объекта, поддерживающего обновление метаданных
  ///
  /// Этот абстрактный класс предоставляет интерфейс для объектов,
  /// которые могут обновлять метаданные (заголовочные и трейлерные).
  /// Используется в контексте вызова RPC методов для модификации
  /// метаданных в процессе обработки запросов.
  const MetadataUpdatable();
